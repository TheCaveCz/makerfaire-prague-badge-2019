...
void cmdInfo(__attribute__((unused)) const char * buf, __attribute__((unused)) const uint8_t len) {
  Serial.println(F("+MFBadge2019/1.0"));
}
...
void cmdSleep(__attribute__((unused)) const char * buf, __attribute__((unused)) const uint8_t len) {
  Serial.println(F("+S"));
  Serial.flush();
  logicSetForceAwake(false);
}

void cmdWake(__attribute__((unused)) const char * buf, __attribute__((unused)) const uint8_t len) {
  Serial.println(F("+W"));
  logicSetForceAwake(true);
}
...
void cmdFill(__attribute__((unused)) const char * buf, __attribute__((unused)) const uint8_t len) {
  uint8_t cnt = 50;
  while (cnt) {
    uint16_t id = random(STORAGE_MAX_ID);
    if (storageIdValid(id) && !storageIdSeen(id)) {
